...
namespace Erp_V1.DAL.DAO
{
    public class SalesDAO : StockContext, IDAO<SALES, SalesDetailDTO>
    {
        ...
        // --- CORRECTED METHOD ---
        // This method is now changed back to return 'bool' to correctly implement the IDAO interface.
        public virtual bool Insert(SALES entity)
        {
            try
            {
                DbContext.SALES.Add(entity);
                // After SaveChanges(), Entity Framework automatically populates the 'ID' of the 'entity' object.
                return DbContext.SaveChanges() > 0;
            }
            catch (DbEntityValidationException ex)
            {
